...
// --- Game Config ---
const int MAX_ROUNDS = 6;      // game ends after clearing 6 rounds
const int FIXED_SPEED = 250;   // constant blink/tone speed (ms), never changes
...
void checkPlayerInput() {
  ...
  if (pressed == sequence[readStep]) {
    ...
    if (readStep == gameStep) {
      ...
      lcd.setCursor(0, 1);
      lcd.print("Cleared: ");
      lcd.print(gameStep + 1);   // shows which round number just finished
      delay(1000);   
      ...
      gameStep++;
      readStep = 0;
      // gameSpeed stays constant now -- no more decrementing
